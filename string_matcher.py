...
class LLMAnswerComparator:
    ...
    def batch_compare(self, responses, oracle_responses, method='ensemble'):
        ...
        if len(responses) != len(oracle_responses):
            raise ValueError("The number of responses and oracle responses must be the same.")
...
if __name__ == "__main__":
    ...
    batch_results = comparator.batch_compare(llm_responses, oracle_responses, method='ensemble')
    for i, (score, correct) in enumerate(batch_results):
        print(f"Batch comparison {i+1} - Similarity score: {score:.2f}, Is correct: {correct}")