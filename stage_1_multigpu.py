...
def load_and_prepare_data(dataset_name, tokenizer, rank, world_size):
    ...
    def tokenize_function(examples):
        ...
        return tokenizer(examples["problem"], truncation=True, padding="max_length", max_length=MAX_LENGTH)
...
def train_score_stage_i(rank, world_size, model, tokenizer, dataloader, num_epochs, learning_rate, beta2):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            correction_inputs = [attempt + self_correction_prompt for attempt in attempt1]
            correction_encodings = tokenizer(correction_inputs, truncation=True, padding="max_length", max_length=MAX_LENGTH, return_tensors='pt')
            # we still need the orig input!
            
            # Combine original input_ids with correction_encodings
            combined_input_ids = torch.cat((input_ids, correction_encodings['input_ids'].to(device)), dim=1)
            combined_attention_mask = torch.cat((attention_mask, correction_encodings['attention_mask'].to(device)), dim=1)

            
            # Second attempt (trained model)
            outputs_2 = model(combined_input_ids, attention_mask=combined_attention_mask)
